...
async function sendWelcomeEmail({ email, token, baseUrl }) {
  const unsubscribeUrl = `${baseUrl}/api/unsubscribe?token=${token}`;
  await sendMail({
    to: email,
    subject: "You're subscribed to Andrew's Story Blog",
    text:
      "You're on the list! You'll get an email whenever a new book or movie review posts.\n\n" +
      `Unsubscribe any time: ${unsubscribeUrl}`,
    html: `
      <div style="font-family: Georgia, 'Times New Roman', serif; color: #2c2c54; max-width: 560px; margin: 0 auto;">
        <h2 style="margin: 0 0 12px;">You're subscribed!</h2>
        <p style="line-height: 1.6;">You'll get an email whenever a new book or movie review posts on Andrew's Story Blog.</p>
        <hr style="margin: 32px 0; border: none; border-top: 1px solid #eee;" />
        <p style="font-size: 0.8rem; color: #999;">
          <a href="${unsubscribeUrl}" style="color: #999;">Unsubscribe</a>
        </p>
...
module.exports = { sendMail, notifySubscribers, sendWelcomeEmail };
